...
if __name__ == '__main__':
    ...
    df = pd.read_excel(r"D:\research\fenlei\dalunwen\JS\画图文件\YCLSTM\S1LSTM2020.xlsx")  # 读取样本数据记得样本要按读取训练的数据那样输入，原始是r"D:\edge_download\training_pointsexcel1.xlsx"
    ...
    # 不同列对应的特征
    vv = X_train_2020[:, 0:12]  # 修改为12个月的VV数据
    vh = X_train_2020[:, 12:24]  # 修改为12个月的VH数据
    SAR_Sum = X_train_2020[:, 24:36]  # 修改为12个月的SAR_Sum数据
    SAR_Diff = X_train_2020[:, 36:48]  # 修改为12个月的SAR_Diff数据
    SAR_NDVI = X_train_2020[:, 48:60]  # 修改为12个月的SAR_NDVI数据
    ...
    def fit(epoch, model, train_loader, test_loader):
        ...
        train_kappa = cohen_kappa_score(all_train_labels, all_train_preds)
        train_ua = np.diag(train_conf_matrix) / (np.sum(train_conf_matrix, axis=1) + 1e-10)
        train_pa = np.diag(train_conf_matrix) / (np.sum(train_conf_matrix, axis=0) + 1e-10)
        # 测试集精度指标
        ...
        test_ua = np.diag(test_conf_matrix) / (np.sum(test_conf_matrix, axis=1) + 1e-10)
        test_pa = np.diag(test_conf_matrix) / (np.sum(test_conf_matrix, axis=0) + 1e-10)
        # 打印详细精度报告
        print("\n=== 详细精度报告 ===")
        ...
        print(f"验证集F1分数: {test_f1:.4f}\n")
        print(f"验证集Kappa系数: {test_kappa:.4f}")

        # 每个类别的UA和PA
        class_metrics = [['类别', '训练UA(%)', '训练PA(%)', '验证UA(%)', '验证PA(%)']]
        for i in range(len(train_ua)):
        ...
        train_f1_per_class = f1_score(all_train_labels, all_train_preds, average=None)
        test_f1_per_class = f1_score(all_test_labels, all_test_preds, average=None)

        # 打印每个类别的F1分数
        print("\n=== 每个类别的F1分数 ===")
        class_f1_metrics = [['类别', '训练集F1分数', '验证集F1分数']]
        for i in range(len(train_f1_per_class)):
            class_f1_metrics.append([
                f"类别 {i + 1}",
                f"{train_f1_per_class[i]:.4f}",
                f"{test_f1_per_class[i]:.4f}"
            ])
        print(tabulate(class_f1_metrics, headers='firstrow', tablefmt='fancy_grid'))
        print("=" * 50)
        ...
        # )
        #
        return epoch_loss, epoch_test_loss, train_accuracy, test_accuracy, train_f1, test_f1, train_kappa, test_kappa

    best_test_loss = float('inf')  # 初始化最佳损失为正无穷大#加两行
    best_model_path = r"D:\research\fenlei\dalunwen\JS\画图文件\YCLSTM\S1YCLSTM2020best_model.pth"  # 保存最佳模型的路径#加两行
    ...
    for epoch in range(epochs):
        epoch_loss, epoch_test_loss, epoch_train_acc, epoch_test_acc, train_f1, test_f1, train_kappa, test_kappa = fit(
            epoch, model, train_loader, test_loader)
    ...
    x = range(0, epochs)
    plt_save_path = r'D:\research\fenlei\dalunwen\JS\画图文件\YCLSTM\rd_lstmS1YC2020'

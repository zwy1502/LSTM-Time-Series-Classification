...
class LSTMClassifier(nn.Module):
    def __init__(self,  input_size=4, hidden_layer_size=40, num_layers=5, output_size=7):
        """
        LSTM二分类任务
        ...
        self.lstm = nn.LSTM(self.input_size, self.hidden_layer_size, self.num_layers, batch_first=True)  # 调用lstm
...
if __name__ == '__main__':
    ...
    model.cuda()
    model.load_state_dict(torch.load(r"E:\nzw\model\LSTM_guang_VH_VV3.pth"))
    model.eval()
    dataset1 = gdal.Open(r"C:\wh\s2\09_7up_9up.tif")
    ...
    pred_img = np.zeros((im111.shape[0], 5, 4))
    pred_img[:, :, 0] = rgb
    pred_img[:, :, 1] =zhishu
    pred_img[:, :, 2] = VH
    pred_img[:, :, 3] = VV
